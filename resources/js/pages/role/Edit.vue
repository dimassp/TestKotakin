...
const item = computed(() => usePage().props.item).value;
...
const submit = () => {
    form.patch(route('role.update', item['id']), {
        preserveScroll: true,
    });
};
</script>
        ...
        <div class="flex h-full flex flex-col gap-4 rounded-xl p-4">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="grid gap-2">
                    ...
                    <Input id="role_name" class="mt-1 block w-full" autocomplete="role_name" required placeholder="Role Name" v-model="form.role_name"/>
                    <InputError :message="form.errors.role_name" />
                </div>

                <div class="grid gap-2">
                    <Label for="role_code">Role Code</Label>
                    <Input id="role_code" type="role_code" class="mt-1 block w-full" autocomplete="role_code" required placeholder="Role Code" v-model="form.role_code"/>
